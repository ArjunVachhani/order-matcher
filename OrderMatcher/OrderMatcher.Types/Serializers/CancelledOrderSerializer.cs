...
public class CancelledOrderSerializer : Serializer
{
    ...
    public static void Serialize(long messageSequence, OrderId orderId, UserId userId, Quantity remainingQuantity, Amount cost, Amount fee, CancelReason cancelReason, int timeStamp, Span<byte> bytes)
    {
        ...
        Amount.WriteBytes(bytes.Slice(costOffset), cost);
        Amount.WriteBytes(bytes.Slice(feeOffset), fee);
    ...
    public static CancelledOrder Deserialize(ReadOnlySpan<byte> bytes)
    {
        ...
        cancelledOrder.Cost = Amount.ReadAmount(bytes.Slice(costOffset));
        cancelledOrder.Fee = Amount.ReadAmount(bytes.Slice(feeOffset));
