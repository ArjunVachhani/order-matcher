...
public class MatchingEngineMarketOrderTests
{
    ...
    [Fact]
    public void Normal_Market_After_Match_Pending_Qty_Cancelled_Sell()
    {
        ...
        mockTradeListener.Verify(x => x.OnTrade(2, 1, 2, 1, false, 10, 1000, null, null, 10000, 20));
        mockTradeListener.Verify(x => x.OnCancel(2, 2, 500, 10000, 50, CancelReason.MarketOrderNoLiquidity));
        mockTradeListener.VerifyNoOtherCalls();
        ...
        mockTradeListener.VerifyNoOtherCalls();
        Assert.Equal(OrderMatchingResult.OrderDoesNotExists, result3);
        Assert.DoesNotContain(order2, matchingEngine.CurrentOrders);
        Assert.Contains(order2.OrderId, matchingEngine.AcceptedOrders);
        Assert.Empty(matchingEngine.Book.BidSide);
        Assert.Empty(matchingEngine.Book.AskSide);
    ...
    [Fact]
    public void Normal_Market_After_Match_Pending_Qty_Cancelled_Buy()
    {
        ...
        mockTradeListener.Verify(x => x.OnAccept(order2.OrderId, order2.UserId));
        mockTradeListener.Verify(x => x.OnTrade(2, 1, 2, 1, true, 10, 1000, 0, 20, null, null));
        mockTradeListener.Verify(x => x.OnCancel(2, 2, 500, 10000, 50, CancelReason.MarketOrderNoLiquidity));
        mockTradeListener.VerifyNoOtherCalls();
        Assert.Equal(OrderMatchingResult.OrderAccepted, result2);
        Assert.DoesNotContain(order2, matchingEngine.CurrentOrders);
    ...
    public void AddOrder_Normal_Market_Matches_Order_With_Open_Orders_For_Sell()
    {
        ...
        mockTradeListener.Verify(x => x.OnAccept(order2.OrderId, order2.UserId));
        mockTradeListener.Verify(x => x.OnTrade(2, 1, 2, 1, false, 10, 500, 0, 25, null, null));
        mockTradeListener.VerifyNoOtherCalls();
        Assert.Equal(OrderMatchingResult.OrderAccepted, result2);
        Assert.DoesNotContain(order2, matchingEngine.CurrentOrders);
        Assert.Contains(order2.OrderId, matchingEngine.AcceptedOrders);
    ...
    public void AddOrder_Normal_Market_Multiple_Matches_Order_With_Open_Orders_For_Sell()
    {
        ...
        mockTradeListener.Verify(x => x.OnTrade(3, 1, 3, 1, false, 10, 500, 0, 25, 10000, 20));
        mockTradeListener.VerifyNoOtherCalls();
    ...
    public void AddOrder_Normal_Market_Matches_Multiple_Order_With_Open_Orders_For_Sell()
    {
        ...
        mockTradeListener.Verify(x => x.OnTrade(3, 2, 3, 2, false, 9, 500, 0, 47.5m, 4500, 9));
        mockTradeListener.VerifyNoOtherCalls();
        Assert.Equal(OrderMatchingResult.OrderAccepted, result3);
        Assert.DoesNotContain(order3, matchingEngine.CurrentOrders);
        Assert.Contains(order3.OrderId, matchingEngine.AcceptedOrders);
    ...
    public void AddOrder_Normal_Market_Matches_Multiple_Order_With_Open_Orders_For_Buy()
    {
        ...
        mockTradeListener.Verify(x => x.OnTrade(3, 2, 3, 2, true, 11, 500, 0, 11, 10500, 52.5m));
        mockTradeListener.VerifyNoOtherCalls();
        Assert.Equal(OrderMatchingResult.OrderAccepted, result3);
        Assert.DoesNotContain(order3, matchingEngine.CurrentOrders);
        Assert.Contains(order3.OrderId, matchingEngine.AcceptedOrders);
    ...
    public void AddOrder_Normal_Market_Cancel_Order_If_Pending_Buy_And_No_Sell_And_No_Match()
    {
        ...
        Assert.Contains(order1.OrderId, matchingEngine.AcceptedOrders);
        Assert.DoesNotContain(order1, matchingEngine.Book.BidSide.SelectMany(x => x));
        Assert.Empty(matchingEngine.Book.BidSide);
        Assert.Empty(matchingEngine.Book.AskSide);
        Assert.Empty(matchingEngine.Book.StopAskSide);
        Assert.Empty(matchingEngine.Book.StopBidSide);
