...
namespace OrderMatcher.Tests
{
    public class MatchingEngineStopLimitGoodTillDateOrderTests
    {
        ...
        [Fact]
        public void AddOrder_CancelsPendingStopLimitOrderBeforeMatch()
        {
        ...
        [Fact]
        public void AddOrder_Stop_Limit_GTD_Trigger_Canacelled()
        {
            ...
            Order order3 = new Order { IsBuy = true, OpenQuantity = 500, Price = 11, OrderId = 3, UserId = 3, StopPrice = 11, CancelOn = 10 };
            ...
            Order order4 = new Order { IsBuy = false, OpenQuantity = 500, Price = 11, OrderId = 4, UserId = 4 };
            ...
            OrderMatchingResult result5 = matchingEngine.AddOrder(order5, 10);

            mockTradeListener.Verify(x => x.OnAccept(order5.OrderId, order5.UserId));
            mockTradeListener.Verify(x => x.OnTrade(5, 4, 5, 4, 11, 500, 0, 11, 5500, 27.5m));
        ...
        [Fact]
        public void AddOrder_Stop_Limit_GTD_Trigger_And_Matches_As_Taker_Buy()
        {
            Order order1 = new Order { IsBuy = false, OpenQuantity = 500, Price = 10, OrderId = 1, UserId = 1 };
            OrderMatchingResult acceptanceResult = matchingEngine.AddOrder(order1, 1);
            ...
            Order order3 = new Order { IsBuy = true, OpenQuantity = 1500, Price = 11, OrderId = 3, UserId = 3, StopPrice = 11, CancelOn = 10 };
            OrderMatchingResult result3 = matchingEngine.AddOrder(order3, 3);
            ...
            Assert.Empty(matchingEngine.Book.StopAskSide);
            Assert.Single(matchingEngine.Book.StopBidSide);
            Assert.Equal(1500, order3.OpenQuantity);
            Assert.Equal((ulong)2, order3.Sequence);
            ...
            mockTradeListener.Verify(x => x.OnTrade(3, 4, 3, 4, 11, 500, 0, 22, null, null));
            ...
            Assert.Equal(0, order5.OpenQuantity);
            Assert.Equal((ulong)0, order5.Sequence);
            Assert.Equal((ulong)4, order3.Sequence);

            matchingEngine.CancelExpiredOrder(10);
            mockTradeListener.Verify(x => x.OnCancel(3, 3, 1000, 5500, 27.5m, CancelReason.ValidityExpired));
            Assert.Equal(OrderMatchingResult.OrderAccepted, result5);
        ...
        public void AddOrder_Stop_Market_Trigger_And_Cancelled()
        {
            ...
            mockTradeListener.Verify(x => x.OnAccept(order5.OrderId, order5.UserId));
            mockTradeListener.Verify(x => x.OnTrade(5, 4, 5, 4, 8, 500, 0, 8, 4000, 20));
            mockTradeListener.Verify(x => x.OnOrderTriggered(3, 3));
